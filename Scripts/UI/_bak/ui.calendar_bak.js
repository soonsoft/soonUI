; (function () {
    ...
    var fontColor = ui.theme.Classes.FontHighlight,
        borderColor = ui.theme.Classes.BorderHighlight,
    ...
    var ctrl = ui.define("ctrls.Calendar", {
        ...
        showTimeLine: function (parent) {
            if (!this.currentTime) {
                this.currentTime = $("<div class='current-time' />");
                this.currentTime.addClass(borderColor).addClass(fontColor)
                    .css("width", timeTitleWidth + "px");
    ...
    YearView.prototype = {
            ...
            for (; i < 12; i++) {
                ...
                div.append(
                    $("<div class='year-month-title' />")
                        .append($("<span />").text((i + 1) + "月").addClass(fontColor)));
            ...
            for (i = 0; i < 6; i++) {
                ...
                for (j = 0; j < 7; j++) {
                    ...
                    if (i == 0 && j < startIndex) {
                        ...
                        if (flag && first == day) {
                            dayVal.addClass("today").addClass(bgColor);
                        }
                        cell.append(dayVal);
                        first++;
                    }
        ...
        addSchedules: function (data, dateField, callback, formatAction) {
            ...
            var action = function (item) {
                ...
                if (i.length == 0) {
                    this.append("<i class='day-marker " + borderColor + "'></i>");
                }
                if (formatAction) {
    ...
    MonthView.prototype = {
        ...
        addSchedules: function (data, dateField, textField, callback) {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                ul.append("<li class='schedule-item " + bgColor + "'><span>" + text + "</span></li>");
                if (callback) {
    ...
    Selector.prototype = {
        _init: function (view, panel, table) {
            ...
            this.selectionBox = $("<div class='selector unselectable click-enabled' />");
            this.selectionBox.addClass(borderColor);
        ...
        getSelectedCells: function () {
            ...
            var boxBorderTopWidth = parseFloat(box.css("border-top-width")),
                top = parseFloat(box.css("top")) + boxBorderTopWidth + 1,
                left = parseFloat(box.css("left")) + boxBorderTopWidth + 1,
                height = box.height() + 1;
            var first = this.getCellByPoint(left, top);
            cells.push(first);
            var count = Math.ceil(height / this.cellHeight),
                table = this.grid[0],
                tableRow = null,
                tableCell = null;
