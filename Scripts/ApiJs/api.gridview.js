;(function () {
     pageLogic.widgetData = {
    	...
    	options: [
            { name: "columns", defaultValue: "空数组", desc: "定义每列的属性，放置了形如{ text: \"列名\"|function, column: \"columnName\"|Array, len: 100, align: \"center|left|right\", handler: function, sort: true|false|function }的对象，其中text是列头显示的文本或者对应方法；column是你要绑定的数据源，可以是数组；len是单元格的长度；align是单元格内容的对齐方式：居中，左对齐，右对齐；handler是调用方法，一般为ui.ColumnStyle里面的方法，如格式化时间；sort为是否排序，true为可以排序，false为不可排序，同样的也可以自定义方法。", isMust: "是" },
            { name: "dataTable", defaultValue: "null", desc: "gridview的数据源。", isMust: "是" },
            { name: "promptText", defaultValue: "\"没有数据\"", desc: "没有数据时所显示的文本。", isMust: "是" },
            { name: "gridId", defaultValue: "null", desc: "gridview父容器的Id。", isMust: "否" },
            { name: "height", defaultValue: "false", desc: "gridview的高度。", isMust: "否" },
            { name: "width", defaultValue: "false", desc: "gridview的宽度。", isMust: "否" },
        ...
        events:[
            {
                ...
                desc: "翻页事件，当点击gridview下方的页码时触发此事件。",
                params: [
        ...
        functionList: [          
            {
                name: "createGridHead(columns)",
                desc: "创建gridview的表头。",
                params: [
                    { name: "columns", type: "数组", desc: "既gridview中的columns属性。" }
                ],
                returnValue: null
            },
            {
                name: "createGridBody(dataTable, rowCount)",
                desc: "根据传入的数据创建表体。",
                params: [
            ...
            },
            {
                ...
                params: [
                    ...
                    { name: "c", type: "对象", desc: "gridview中一列的属性对象。" }
                ],
            ...
            },
            {
                ...
                desc: "获取当前选择项的数据，当gridview设置为单选才可以使用。",
                params: [],
                returnValue: "当gridview设置为行选中时，返回一个对象，该对象包含当前选中行的rowData对象（该行的所有数据）和rowIndex。"
                    + "当gridview设置为单元格选中时，返回一个对象，"
                    + "该对象包含当前选中行的rowData对象（该行的所有数据）和rowIndex（行索引），还包含cellIndex（列索引）和columns（单元格中的数据）。"
            },
            {
                name: "getMultipleSelection()",
                desc: "获取当前多选项中的数据，当gridview设置为可多选才可以使用。",
                params: [],
                returnValue: "数组，里面包含所有选中的对象。当gridview设置为行选中时，对象包含当前选中行的rowData对象和rowIndex。"
                    + "当gridview设置为单元格选中时，对象包含当前选中行的rowData对象（该行的所有数据）和rowIndex（行索引），还包含cellIndex（列索引）和columns（单元格中的数据）。"
            },
            {
                name: "count()",
                desc: "获取当前页面上显示的记录数目。",
            ...
            {
                name: "currentUp()",
                desc: "当前选择的行上移。在gridview为多选时不可用。",
                params: [],
                returnValue: null
            },
            {
                name: "currentDown()",
                desc: "当前选择的行下移。在gridview为多选时不可用。",
                params: [],
                returnValue: null
            },
            {
            ...
            },
            {
                ...
                desc: "设置表格内容显示区域的高度。常用于布局计算layout中设置gridview的高度。",
                params: [
            ...
            },
            {
                ...
                desc: "设置表格的大小。常用于布局计算layout中设置gridview的宽和高。",
                params: [
                    { name: "height", type: "数值", desc: "高度，代表像素，不需要加px。" },
                    { name: "width", type: "数值", desc: "宽度，代表像素，不需要加px。" }
                ],
